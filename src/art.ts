...
export const typeDescriptions = Object.freeze(
  ['paper art', 'fabric art', 'furnishing', 'painting', 'scrimshaw or woodwork', 'ceramics', 'glasswork', 'stonework', 'metalwork', 'magical']
);
...
export const workDescriptions = Object.freeze(
  ['awfully executed', 'poorly executed', 'below average execution', 'average execution',
  'above average execution', 'good execution', 'excellent execution', 'brilliant execution', 'masterpiece']
);
...
export const conditionDescriptions = Object.freeze(
  ['badly damaged', 'damaged', 'worn', 'average condition', 'good condition', 'excellent condition', 'near-perfect condition', 'perfect condition', 'flawless condition']
);
...
export const subjectDescriptions = Object.freeze(
  ['abstract', 'monster', 'human or demi-human', 'natural', 'historical', 'religious', 'wealthy/noble', 'royalty']
);
...
const valueLookup = Object.freeze(
  [1, 10, 20, 30, 40, 50, 60, 70, 85, 100, 125, 150, 200, 250, 325, 400, 500, 650, 800, 1000, 1250, 1500, 2000, 2500, 3000, 4000, 5000, 6000, 7500, 10000,
  12500, 15000, 20000, 25000, 30000, 40000, 50000, 60000, 70000, 85000, 100000, 125000, 150000, 200000, 250000, 300000, 400000, 500000, 650000, 800000, 1000000]
);

function getQuality(renown: Renown) {
  const baseMaterial = Util.tableLookup(MaterialQuality, qualityThresholds, rollDie(100));
  const baseWork = Util.tableLookup(WorkQuality, qualityThresholds, rollDie(100));
  const materialQuality = Util.limitToRange(baseMaterial + renown - 3, MaterialQuality.Awful, MaterialQuality.Unique);
  const workQuality = Util.limitToRange(baseWork + renown - 3, WorkQuality.Awful, WorkQuality.Masterpiece);
  return { materialQuality, workQuality };
}

function getAge(type: Type): number {
  const baseAge = roll('5d4', { penetration: true }) * rollDie(20, 0, { penetration: true });
  ...
  const min = modifiedRange === AgeRange.Zero ? 5 : ageThresholds[modifiedRange - 1];
  return Util.randInt(min, max);
}

function getCondition(type: Type): Condition {
  const baseCondition = Util.tableLookup(Condition, conditionThresholds, rollDie(100));
  const modifier = conditionModifiersByType[type];
  return Util.limitToRange(baseCondition + modifier, Condition.BadlyDamaged, Condition.Flawless);
}
...
export function randomArt(): IArt {
  ...
  const value = getValue(renown, size, materialQuality, workQuality, age, condition, subject);

  return { type, renown, size, materialQuality, workQuality, age, condition, subject, value }
}
